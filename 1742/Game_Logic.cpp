...
void updateParticleList(OBJECT **listHeader);
...
void moveAllObjects()
{
   ...
   while(theBullet != NULL)
	...
	moveObjectList(bulletHeader);
	moveObjectList(enemyBulletHeader);
   moveObjectList(enemyHeader);
	moveObjectList(alliesHeader);
	evaluateGroundCollision(&theShip);
   evaluateGroundCollision(enemyHeader);
	cleanObjectList(&bulletHeader);
	cleanObjectList(&enemyBulletHeader);
	cleanObjectList(&alliesHeader);
   checkForDeath(&enemyHeader);
   checkForDeath(&enemyHeader);
	cleanObjectList(&enemyHeader);
	moveObjectList(particleHeader);
	updateParticleList(&particleHeader);
   updateAirStrikes();
}
